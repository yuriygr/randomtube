  ...
  <section ref="player" class="player" v-on="$playerEvents" tabindex="0">
    ...
    <div :class="[ 'player__bar', { 'player__bar--zen': zenMode } ]" @contextmenu="$event.preventDefault()">
      <bar-buttons>
        ...
        <bar-item icon="external-link" :title="$t('player.bar.source')" :disabled="!currentFile.source" component="a" :href="currentFile.source" target="_blank" />
        <bar-item icon="play-list" :title="$t('player.bar.playlist')" :disabled="sources.length == 0" @click.exact="showPlaylistModal" />
...
import {
  PlayerOverlay, PlayerError
} from '@/components/player'
import {
  BarButtons, BarItemReaction, BarItem, BarDivider
} from '@/components/player-bar'

let PlaylistsModal = defineAsyncComponent(() => import("@/components/modals/Playlist.vue"))
let ReportItemModal = defineAsyncComponent(() => import("@/components/modals/ReportItem.vue"))
let KeybindingsModal = defineAsyncComponent(() => import("@/components/modals/Keybindings.vue"))
  
export default {
  name: 'gallery',
  components: { PlayerOverlay, PlayerError, BarItem, BarDivider, BarButtons, BarItemReaction },
  data() {
    return {
      default: {
        chan: '2ch.hk',
  ...
  methods: {
     ...
     */
    showPlaylistModal() {
      this.$modals.show(PlaylistsModal)

      this.$gtag.event('player', {
        'event_category': 'navigation',
        'event_label': 'showPlaylistModal'
      })
    },
